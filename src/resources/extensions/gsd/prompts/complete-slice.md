...
11. Call `gsd_slice_complete` with camelCase parameters like milestoneId, sliceId, the slice summary fields, and `uatContent`. Do NOT manually mark the roadmap checkbox — the tool writes to the DB and renders the ROADMAP.md projection automatically.
...
**You MUST call `gsd_slice_complete` with camelCase parameters including `sliceId`, `milestoneId`, and `uatContent` before finishing. The tool persists to both DB and disk and renders `{{sliceSummaryPath}}` and `{{sliceUatPath}}` automatically.**
