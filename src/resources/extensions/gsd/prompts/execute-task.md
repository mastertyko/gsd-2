...
20. Call `gsd_task_complete` with camelCase parameters like milestoneId, sliceId, taskId, and the completion fields describing what was accomplished. This is your final required step — do NOT manually edit PLAN.md checkboxes. The tool marks the task complete, updates the DB, and renders PLAN.md automatically.
...
**You MUST call `gsd_task_complete` AND write `{{taskSummaryPath}}` before finishing.**
